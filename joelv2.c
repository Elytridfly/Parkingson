...
static void buttonOne(){
	...
	// Store the ticket number globally
	stored_ticket = ticketNum;
	is_ticket_valid = 1;
...
static void buttonHex(){
	...
	// Check if there's a valid ticket in the system
	if(!is_ticket_valid){
		display_error_message("No active ticket!");
		usleep(2000000);
		return; // Return to main menu
	}
	
	// Get ticket number input from user
	lcd_writecmd(0x01);
	lcd_writecmd(0x80);
	LCDprint("Enter Ticket #:");
	lcd_writecmd(0xC0);
	
	entered_ticket = get_ticket_input();
	
	// Validate ticket number
	if(entered_ticket != stored_ticket){
		display_error_message("Invalid Ticket!");
		usleep(2000000);
		// Return to appropriate menu based on inOut mode
		if (inOut == 0){
			system("DISPLAY=:0.0 pqiv -f /tmp/entry.jpg &");		
			sleep(2);	
			initlcd();
	...
	// Valid ticket - proceed with exit
	...
	create_custom_chars();
	lcd_writecmd(0x80);
	LCDprint("Ticket Valid!");
	lcd_writecmd(0xC0);
	LCDprint("Car Exiting...");
	usleep(1000000);
	animate_car_exiting();

	// Clear the ticket - it's been used
	stored_ticket = 0;
	is_ticket_valid = 0;
...
// Function to get 6-digit ticket number input from keypad
static int get_ticket_input(){
    char input_str[7] = {0}; // 6 digits + null terminator
    int digit_count = 0;
    unsigned char key;
    ...
    while(digit_count < 6){
        ...
        if(key >= '0' && key <= '9'){
            ...
            lcddata(key); // Display the digit
            digit_count++;
        }
        // Handle backspace with '*' key
        else if(key == 'A' && digit_count > 0){
            digit_count--;
            input_str[digit_count] = '\0';
            lcd_writecmd(0xC0 + digit_count); // Move cursor back
            lcddata(' '); // Clear the digit
            lcd_writecmd(0xC0 + digit_count); // Move cursor back again
        }
        // Cancel input with '#' key
        else if(key == 'B'){
            return -1; // Invalid input
        }
    }
    
    input_str[6] = '\0'; // Ensure null termination
...
static void display_error_message(char* message){
    ...
    lcd_writecmd(0x80); // First line
    LCDprint(message);
    lcd_writecmd(0xC0); // Second line
